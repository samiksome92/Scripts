...
def resize_imgs(img_files: List[str], img_ids: List[str], out_dir: str, height: int = 300) -> None:
    ...
    """
    progress_bar = tqdm(
        zip(img_files, img_ids), bar_format="Resizing Images    |{bar:20}| {n_fmt}/{total_fmt}", total=len(img_files)
    )
...
def write_html(
    ...
    """
    # HTML start.
    html = "<!DOCTYPE html>" "<html>" "<head>" "<title>Gallery</title>" '<meta charset="utf-8">'
    ...
    # End head section.
    html += "</head>" f'<body><div id="gallery">'
    ...
    # Image viewer.
    html += (
        '<div id="viewer">' '   <img id="viewerImg">' '   <div id="left"></div>' '   <div id="right"></div>' "</div>"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "-b", "--browser", help="Custom browser command (arguments supported)", nargs=argparse.REMAINDER
    )
    ...
    status = create_gallery(
        args.dir_path, out_dir, args.randomize, args.height, args.padding, args.no_resize, args.port
    )
    if status:
